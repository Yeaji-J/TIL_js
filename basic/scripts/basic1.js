...
let user_age=10;
let user_height=130;
let user_residence="인천"
console.log(user_name,user_age,user_height,user_residence);
//Q. 가을은 내년에 11살입니다.
/* user_age=11; */
/* console.log(user_name,'은 내년에',user_age+1,'살입니다.'); */
console.log(`${user_name}은 내년에 ${user_age+1}살입니다.`)
//Q. 현재는 인천에 살지만
console.log('현재는',user_residence,'에 살지만');
//Q. 다음 달 서울로 이사갑니다.
user_residence='서울'
console.log('다음 달',user_residence,'로 이사갑니다.');
console.log(`다음 달 ${user_residence}로 이사갑니다`)
console.log('-------------------------------------')
//장바구니 변수
...
cart=null; //cart=''; // 장바구니 비우기 (삭제버튼 클릭 시;; 나중에 이벤트와 함께 설정)
console.log(cart);
...
/* const webStudy3; //undefined => 'figma'
webStudy3='figma'; */
const webStudy3 = 'figma'; //생성 & 대입 동시처리
//자바스크립트 기초 함수
//1. log(); 기록 실행 함수
//2. typeof(); 데이터타입 실행 함수
//3. print(); 인쇄 실행 함수 --> 나중에 인쇄버튼이랑 함께 이벤트 적용하기!
//4. prompt(); 질문답변 실행 함수
...
/* const q1 = Number(prompt('1+1은?')); //const q1이 1번, prompt('1+1은?')이 2번, number()가 3번, = 이 4번으로 적용됨 //Number():숫자 변경 함수
console.log(q1);
console.log(typeof(q1)); //number */
/* confirm(q1); */

//const kurly_order_num = Number(prompt('주문수량은?'));
//const kurly_price = 10000;
//const kurly_total = kurly_order_num*kurly_price /* 변수를 따로 만들면 유지보수에도 좋고, 실수를 방지할 수 있다 */
