...
for(let i=0; i<5; i++){console.log(array[i])}
...
/* for(let i=0; i<=dan2Li.length; i++){
    if(i%2==1){dan2Li[i].style.backgroundColor = 'yellow'}
    else{dan2Li[i].style.backgroundColor = 'skyblue'}
}
 */
// 홀수 li w50 h20 bg노랑 color 파랑
// 홀수 li w50 h20 bg파랑 color 노랑
...
// 활용 예) 사용자로부터 올바른 입력값을 받을 때까지 반복
...
alert('정답입니다') */
/* let cssQ = prompt('ox퀴즈 : mixin을 만들 때에는 앞에 달러사인($)을 붙여야한다')
while(cssQ != 'x'){
    cssQ = prompt('ox퀴즈 : mixin을 만들 때에는 앞에 달러사인($)을 붙여야한다')
}
alert('정답입니다람쥐') */

/* while(true){
    const qa = prompt ('자바스크립트는 동적언어이다(o/x)')
    if(qa != 'o' && qa != 'x' && qa != 'O' && qa != 'X'){ // 잘못된 답안 조건문
        alert('잘못된 입력입니다. o,x만 입력해주세요.');
        continue; // 정답, 오답 세트로 내려가지 않고 다시 위 prompt로 돌아간다
    } 
    if(qa === 'o' || qa ==='O'){
        alert('정답입니다'); break;
...
let li99 = '';
for(let i=2; i<10; i++){ // 단 for 시작
    for(let j=1; j<10; j++){
        li99 += `<li>${i} X ${j} x = ${i*j}</li>`
